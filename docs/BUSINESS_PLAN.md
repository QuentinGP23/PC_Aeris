# Business Plan — PC Aeris

**Le PC sur-mesure, enfin accessible à tous.**

---

## Executive Summary

**PC Aeris** est une plateforme web qui démocratise l'accès au PC sur-mesure en combinant un configurateur intelligent, une IA de compatibilité et une transparence totale sur les prix.

| Élément               | Description                                                                |
| --------------------- | -------------------------------------------------------------------------- |
| **Problème**          | Acheter un PC adapté est complexe, chronophage et source d'erreurs         |
| **Solution**          | Plateforme tout-en-un : questionnaire, configurateur IA, prix transparents |
| **Cible**             | Particuliers et professionnels individuels (gamers, créatifs, étudiants)   |
| **Modèle économique** | Offres de montage à prix fixe (79–199 €), composants à prix coûtant ; services à venir |
| **Différenciation**   | Compatibilité garantie par IA + fourchettes de prix marché                 |

---

## 1. Présentation du projet

### Vision

Permettre à chacun d'obtenir le PC parfait pour ses besoins — sans stress, sans erreur, au juste prix.

### Mission

Simplifier l'achat de PC sur-mesure grâce à la technologie et la transparence.

### Valeurs

- **Simplicité** — une expérience accessible à tous les niveaux
- **Transparence** — prix clairs, pas de surprises
- **Expertise** — configurations optimisées et garanties compatibles

---

## 2. Produits et services

| Offre                    | Description                                         | Cible        |
| ------------------------ | --------------------------------------------------- | ------------ |
| **Questionnaire guidé**  | Quelques questions → configuration recommandée      | Débutants    |
| **Configurateur IA**     | Sélection composants avec suggestions intelligentes | Connaisseurs |
| **PC prêts à l'emploi**  | Configurations assemblées et testées                | Tous         |
| **Offres de montage**    | Montage par nos soins — Essentiel 79 € / Confort 129 € / Premium 199 € | Cœur du revenu |
| **Garantie étendue**     | Extension de garantie sur PC assemblés              | Option       |

---

## 3. Marché

| Indicateur                            | Valeur                                   |
| ------------------------------------- | ---------------------------------------- |
| Marché du PC en France                | ~4 milliards €/an                        |
| Croissance segment gaming/workstation | +8 à 10%/an                              |
| Part du sur-mesure                    | En croissance, peu d'acteurs spécialisés |

### Cibles prioritaires

| Segment             | Caractéristiques             | Panier moyen    |
| ------------------- | ---------------------------- | --------------- |
| Gamers débutants    | 16-30 ans, premier PC        | 800 - 1 500 €   |
| Gamers confirmés    | Upgrades réguliers           | 1 500 - 3 000 € |
| Créatifs/Freelances | Montage, 3D, dev             | 1 200 - 2 500 € |
| Étudiants tech      | Budget serré, besoins précis | 600 - 1 200 €   |
| Pros individuels    | Indépendants, consultants    | 1 000 - 2 500 € |

---

## 4. Concurrence et positionnement

| Concurrent             | Forces                     | Faiblesses                         |
| ---------------------- | -------------------------- | ---------------------------------- |
| LDLC / Materiel.net    | Notoriété, catalogue large | UX technique, pas de conseil IA    |
| TopAchat               | Prix compétitifs           | Interface datée                    |
| Amazon                 | Logistique, prix           | Pas de conseil, jungle de produits |
| Configurateurs marques | Intégration verticale      | Limités à une marque               |

### Positionnement PC Aeris

```
          EXPERTISE
              ▲
              │
    LDLC      │      PC AERIS
              │        ★
              │
 ─────────────┼─────────────► SIMPLICITÉ
              │
   TopAchat   │    PC pré-montés
              │     (grande distrib)
              │
```

**PC Aeris occupe le quadrant inexploité : expertise + simplicité.**

---

## 5. Modèle économique

### Sources de revenus

Le client paie les composants **au meilleur prix du marché** (sans marge de notre part) **+ une offre de montage à prix fixe**. Le montage est notre revenu principal — pas un pourcentage sur les composants.

| Source                        | Prix          |
| ----------------------------- | ------------- |
| Montage — **Essentiel**       | 79 €          |
| Montage — **Confort**         | 129 €         |
| Montage — **Premium**         | 199 €         |
| Garantie étendue (à venir)    | 30 - 80 €     |
| Abonnement Pro (à venir)      | 5 - 10 €/mois |

> Revenu moyen par commande ≈ **120 €** (mix des offres de montage).

### Projections financières (réalistes)

| Année | Commandes/mois | Panier moyen | CA annuel | Marge brute |
| ----- | -------------- | ------------ | --------- | ----------- |
| N+1   | ~6 (montée)    | 1 200 €      | ~86 K€    | 10 - 13 K€  |
| N+2   | ~16            | 1 250 €      | ~240 K€   | 29 - 36 K€  |
| N+3   | ~32            | 1 300 €      | ~500 K€   | 60 - 75 K€  |

> Hypothèses prudentes pour une marque sans notoriété initiale. La croissance dépend du bouche-à-oreille, du SEO et des partenariats créateurs.

_Hypothèses conservatrices avec montée en charge. Marge brute ≈ l'offre de montage (~10 % du CA, composants à prix coûtant)_

---

## 6. Stratégie marketing

### Acquisition

| Canal               | Actions                            | Budget estimé             |
| ------------------- | ---------------------------------- | ------------------------- |
| **SEO**             | Guides, comparatifs, tutoriels     | Temps + 200 €/mois outils |
| **Réseaux sociaux** | TikTok, Instagram, Twitter/X       | 500 €/mois                |
| **YouTube/Twitch**  | Partenariats créateurs tech/gaming | 1 000 €/mois              |
| **Communauté**      | Discord, forum, partage configs    | Temps                     |

### Fidélisation

- Espace client personnalisé
- Alertes prix sur composants
- Programme de parrainage
- Newsletter tech

---

## 7. Organisation et équipe

### Phase de lancement (N+1)

| Rôle                   | Profil                          | Statut                 |
| ---------------------- | ------------------------------- | ---------------------- |
| Fondateur / CEO        | Vision, stratégie, commercial   | Temps plein            |
| Développeur full-stack | Plateforme, IA                  | Temps plein ou associé |
| Expert hardware        | Base de données, compatibilités | Temps partiel          |

### Phase de croissance (N+2/N+3)

- +1 développeur
- +1 responsable marketing
- +1 support client
- Partenaires assemblage

---

## 8. Roadmap

| Phase          | Période        | Objectifs                                               |
| -------------- | -------------- | ------------------------------------------------------- |
| **MVP**        | T1 2026        | Configurateur de base, authentification, BDD composants |
| **V1**         | T2-T3 2026     | IA de suggestion, système de prix, commandes            |
| **Croissance** | T4 2026 - 2027 | Marketing, partenariats, optimisations                  |
| **Expansion**  | 2027+          | App mobile, abonnement Pro, internationalisation        |

---

## 9. Besoins de financement

### Phase d'amorçage (6-12 mois)

| Poste                    | Montant      |
| ------------------------ | ------------ |
| Développement plateforme | 15 000 €     |
| Marketing lancement      | 10 000 €     |
| Infrastructure technique | 3 000 €      |
| Juridique / Comptabilité | 2 000 €      |
| Trésorerie / Imprévus    | 5 000 €      |
| **Total**                | **35 000 €** |

### Sources de financement envisagées

- Apport personnel
- Love money (proches)
- Prêt d'honneur (Réseau Entreprendre, Initiative France)
- BPI France (garantie, avance remboursable)
- Concours startups

---

## 10. Indicateurs de succès (KPIs)

| Catégorie        | KPI                    | Objectif N+1 |
| ---------------- | ---------------------- | ------------ |
| **Acquisition**  | Visiteurs uniques/mois | ~1 500       |
| **Conversion**   | Taux de conversion     | 0,5 %        |
| **Satisfaction** | NPS                    | > 40         |
| **Financier**    | CA mensuel moyen (N+1, en montée) | ~7 000 €     |
| **Rétention**    | Taux de recommandation | 30 %         |

---

## 11. Risques et mitigations

| Risque                | Probabilité | Impact | Mitigation                               |
| --------------------- | ----------- | ------ | ---------------------------------------- |
| Faible notoriété      | Élevée      | Élevé  | Marketing digital agressif, partenariats |
| Marges insuffisantes  | Moyenne     | Élevé  | Services à valeur ajoutée                |
| Pénurie composants    | Moyenne     | Moyen  | Multi-sourcing, alertes                  |
| Concurrence accrue    | Moyenne     | Moyen  | Différenciation UX/IA                    |
| Défaillance technique | Faible      | Élevé  | Tests, monitoring, backups               |

---

## Conclusion

PC Aeris répond à un besoin réel sur un marché porteur, avec une proposition de valeur différenciante.

**Forces du projet :**

- ✅ Niche peu exploitée (sur-mesure simplifié)
- ✅ Modèle asset-light (pas de stock)
- ✅ Technologie différenciante (IA)
- ✅ Fondations techniques opérationnelles

**Prochaines étapes :**

1. Finaliser le MVP (configurateur + IA)
2. Recruter les premiers utilisateurs beta
3. Itérer sur les retours
4. Lancer la V1 commerciale

---

_Document de travail — PC Aeris — Janvier 2026_
